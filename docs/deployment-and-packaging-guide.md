...
This document outlines the requirements and optimal strategies for compiling, packaging, and deploying audio plugins (VST3, AU, AAX, CLAP) across multiple platforms (Windows, macOS, Linux).
...
- **JUCE Configuration:** Enable `AU` in the CMake `FORMATS` list. Ensure the plugin `AU_MAIN_TYPE` (e.g., `aufx` for effects, `aumu` for instruments) and a 4-character manufacturer code are set.
- **Validation:** Use Apple's `auval` tool via terminal: `auval -v aufx <plugin_code> <manu_code>`.
...
- **Prerequisites:** Standard C++ compiler (MSVC, Xcode, GCC/Clang). JUCE 8 has native CLAP support.
- **JUCE Configuration:** Enable `CLAP` in the CMake `FORMATS` list.
- **Validation:** Use `clap-validator` or test in Bitwig Studio / Reaper.

### Linux Builds (VST3 / CLAP)
...
1. **Windows (Inno Setup):**
   ...
   - Copies `.aaxplugin` to `C:\Program Files\Common Files\Avid\Audio\Plug-Ins\`.
...
- **On Windows (VST3, CLAP):** 
  ...
  - **DAW Loading:** Once installed, Windows DAWs (FL Studio, Ableton, Studio One) generally do **not** block unsigned `.vst3` or `.clap` DLLs. The plugin will load normally.

- **On macOS (VST3, AU, CLAP):**
  ...
  - **Mandatory User Action:** To make unsigned plugins work on macOS, the user **must** open the Terminal and run a specific command to strip the quarantine attribute from the installed plugins:
    ...
    sudo xattr -rd com.apple.quarantine /Library/Audio/Plug-Ins/CLAP/YourPlugin.clap
    ```
...
- **macOS Runner:** Compiles AU/VST3/CLAP, builds the PKG, and attaches it to a GitHub Release.
- **Windows Runner:** Compiles VST3/CLAP, builds the Inno Setup EXE, and attaches it to the release.
