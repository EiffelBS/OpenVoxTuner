...
[スクリーンショット](#スクリーンショット) &bull;
[機能](#機能) &bull;
[なぜ OpenVoxTuner なのか](#なぜ-openvoxtuner-なのか) &bull;
[リポジトリ構造](#リポジトリ構造) &bull;
[ライセンス](#ライセンス) &bull;
[プロジェクトを支援する](#プロジェクトを支援する) &bull;
[開発者ライセンス](#開発者ライセンス) &bull;
[コントリビュート](#コントリビュート) &bull;
[ビルド](#ビルド) &bull;
[ドキュメント](#ドキュメント)
...
- **グラフィックモード** — 独自のピッチカーブを描画（グラフィカルエディター：スナップ、グリッド、コピー/ペースト、Undo/Redo）
...
- ARA2 サポート — DAW タイムラインからキー、スケール、拍子を読み取ります
- 拍子に敏感な小節ルーラーと自動スクロール
- マルチ拍子対応（プロジェクト中盤の拍子変更）
- DAW タイムラインに同期した波形オーバーレイ

> ARA2 はプラグインを DAW タイムライン（キー/スケール、拍子、プレイヘッド、波形）に同期させます。OpenVoxTuner はあくまで**リアルタイムエフェクト**であり、専用ピッチエディターのようなノート単位のオフライン編集は提供しません。
...
- **ネイティブ ARA2** — キー、スケール、拍子が DAW プロジェクトから直接読み取られるため、手動設定なしでアレンジメントに追従します。
- **本物の歌声のために設計** — YIN ピッチ検出、フォルマント保持 PSOLA、グラフィカルなカーブエディターは、単なるコンセプト実証デモではなく、歌の演技のニュアンスに合わせて調整されています。
